    ...
    public class _StatisticsWidgetComponentPartial:ViewComponent
    {
        ...
        public IViewComponentResult Invoke()
        {
           ViewBag.categoryCount= _context.Categories.Count();
            ...
            ViewBag.productMaxPriceProductName = _context.Products.Where(x=>x.ProductPrice == (_context.Products.Max(p=>p.ProductPrice))).Select(y=>y.ProductName).FirstOrDefault();

            ViewBag.productMinPriceProductName = _context.Products.Where(x => x.ProductPrice == (_context.Products.Min(p => p.ProductPrice))).Select(y => y.ProductName).FirstOrDefault();

            ViewBag.totalStock = _context.Products.Sum(x => x.ProductStock);
